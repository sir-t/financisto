...
public class CurrencyTest extends AbstractDbTest {
    ...
    @Test
    public void should_format_symbol_according_to_the_selected_format() {
        //given
        Currency c = new Currency();
        c.decimals = 1;
        c.decimalSeparator = "'.'";
    ...
    @Test
    public void should_reset_default_flag() {
    ...
    @Test
    public void should_return_home_currency() {
    ...
    @Test
    public void should_return_empty_currency_if_home_is_not_set() {
        CurrencyBuilder.withDb(db).name("USD").title("Dollar").symbol("$").create();
        assertEquals("", db.getHomeCurrency().name);
    }

    @Test
    public void should_return_empty_currency_if_there_are_no_currencies() {
        assertEquals("", db.getHomeCurrency().name);
    }

    @Test
    public void should_set_home_currency_if_it_has_not_been_set_and_the_same_currency_is_used_in_all_accounts() {
        CurrencyBuilder.withDb(db).name("USD").title("Dollar").symbol("$").create();
